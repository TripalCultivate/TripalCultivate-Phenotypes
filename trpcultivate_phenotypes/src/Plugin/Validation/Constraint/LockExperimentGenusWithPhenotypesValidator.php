...
class LockExperimentGenusWithPhenotypesValidator extends ConstraintValidator {
  ...
  public function validate($value, Constraint $constraint): void {
    ...
    foreach ($value->getFields() as $field) {
      ...
      if ($table != $constraint->field_req['base_table']) {
        continue;
      }
      ...
      if ($constraint->term_namespace == $constraint->field_req['term'] . ' (' . $idspace . ':' . $accession . ')') {
        $exp_germgenus = $field->getValue();
    ...
    if ($exp_germgenus) {
      ...
      if (count($pheno_configgenus) > 0) {
        $exp_germgenus = array_filter(
          array_column($field->getValue(), 'value')
        );
        ...
        foreach ($pheno_configgenus as $genus) {
          ...
          $experiment_id = $this->context
            ->getObject()
            ->getEntity()
            ->getBackendRecordId('chado_storage');
